...
const resolvers = {
  ...
  OCLEvent: {
    ...
    async standingsJpgURL(event) {
      const jpgURL = await (0, _db.executeSelectOne)(`SELECT standingsJpgURL FROM eventExtra WHERE id = $eventId`, {
        $eventId: event.id
      }, 'standingsJpgURL');
...
`;
  ...
  Card: {
    ...
    async ownedMTGOCard(card, {
      ...
      if (!wishlistOnly) {
        ownedCard = await (0, _db.executeSelectOne)(sql.selectOwnedMTGOCardByName, {
          $cardName: card.name
        });
      }
