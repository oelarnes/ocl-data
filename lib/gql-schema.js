const {OCLEvent, Player, exportDecklists, replaceRoster} = require('./data-model');
...
const typeDefs = `
    ...
    type Event {
        ...
        eventName: String!
        startDate: Date!
        rankedRoster: [RosterEntry!]!
        prizeType: PrizeType!
        prizeSpec: [PrizeSpec!]
        roundType: RoundType!
        tournamentType: TournamentType!
        results: [RoundResult!]!
        finishedRound: RoundDesc
    }
    type Query {
        getPlayers: [Player!]!
        getEvents: [Event!]!
...
const resolvers = { 
    ...
    Mutation: {
        ...
        deleteEvent: (_, {eventId}) => OCLEvent.remove({_id: new ObjectId(eventId)}).then(({deletedCount}) => deletedCount),
        updateEvent: (_, {eventId, eventInput}) => OCLEvent.findOne({_id: new ObjectId(eventId)})
            .then((event) => Object.assign(event, eventInput).save()),
        replaceRoster: (_, {eventId, playerIds}) => OCLEvent.findOne({_id: new ObjectId(eventId)})
            .then(event => replaceRoster(event, playerIds).save())
    }
};
