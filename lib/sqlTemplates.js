...
const dropEventExtraTable = `DROP TABLE IF EXISTS eventExtra;`;
exports.dropEventExtraTable = dropEventExtraTable;
const dropEntryTable = 'DROP TABLE IF EXISTS entry;';
exports.dropEntryTable = dropEntryTable;
const dropEntryExtraTable = `DROP TABLE IF EXISTS entryExtra;`;
exports.dropEntryExtraTable = dropEntryExtraTable;
...
const createEventExtraTable = `
CREATE TABLE IF NOT EXISTS eventExtra(
    id TEXT PRIMARY KEY,
    standingsJpgURL TEXT
);
`;
exports.createEventExtraTable = createEventExtraTable;
...
`;
exports.createEntryExtraTable = createEntryExtraTable;
...
const selectPlayersByNameOrHandleSearch = `SELECT * FROM player WHERE fullName LIKE '%' || $byName || '%' OR discordHandle = $byHandle`;
...
const selectStandingsAllTime = `SELECT *, rank as allTimeRank FROM (
    SELECT 
        ROW_NUMBER () OVER (
            ORDER BY entries.qps DESC, entries.trophies DESC, p.wins DESC, p.losses ASC
        ) rank,
        p.playerId AS playerId, 
        p.wins AS matchWins, 
        p.losses AS matchLosses, 
        entries.qps AS qps, 
        entries.trophies AS trophies,
        'All Time' as season
    FROM (
        SELECT playerId, SUM(wins) as wins, SUM(losses) as losses FROM (
            SELECT p1Id as playerId, p1MatchWin AS wins, p2MatchWin AS losses FROM pairing
                UNION ALL
            SELECT p2Id as playerId, p2MatchWin AS wins, p1MatchWin AS losses FROM pairing
        ) GROUP BY playerId
    ) p
    JOIN (
        SELECT playerId, SUM(qpsAwarded) AS qps, SUM(CASE finalPosition WHEN 1 THEN 1 ELSE 0 END) AS trophies FROM entry GROUP BY playerId
    ) entries
    ON entries.playerId = p.playerId
...
const selectStandingsBySeason = `SELECT * FROM (
    SELECT
        ROW_NUMBER () OVER (
            ORDER BY entries.qps DESC, p.wins DESC, p.losses ASC, ats.rank ASC
        ) rank,
        p.playerId AS playerId,
        p.wins AS matchWins,
        p.losses AS matchLosses,
    ...
    FROM (
        SELECT playerId, SUM(wins) as wins, SUM(losses) as losses FROM (
            SELECT p1Id as playerId, p1MatchWin AS wins, p2MatchWin AS losses FROM pairing JOIN event ON pairing.eventId = event.id WHERE event.season = $season
                UNION ALL
            SELECT p2Id as playerId, p2MatchWin AS wins, p1MatchWin AS losses FROM pairing JOIN event ON pairing.eventId = event.id WHERE event.season = $season
        ) GROUP BY playerId
    ) p
    JOIN (
        SELECT playerId, SUM(qpsAwarded) AS qps, SUM(CASE finalPosition WHEN 1 THEN 1 ELSE 0 END) AS trophies 
        FROM entry JOIN event ON entry.eventId = event.id WHERE event.season = $season GROUP BY playerId
    ) entries
    ON entries.playerId = p.playerId
    JOIN (
        SELECT 
            ROW_NUMBER () OVER (
                ORDER BY entries.qps DESC, entries.trophies DESC, pa.wins DESC, pa.losses ASC
            ) rank, pa.playerId AS playerId
        FROM (
            SELECT playerId, SUM(wins) as wins, SUM(losses) as losses FROM (
                SELECT p1Id as playerId, p1MatchWin AS wins, p2MatchWin AS losses FROM pairing
                    UNION ALL
        ...
        ON entries.playerId = pa.playerId
    ) ats
    ON ats.playerId = p.playerId
) t  WHERE rank > $after LIMIT $howMany`;
exports.selectStandingsBySeason = selectStandingsBySeason;
const selectStandingsWithDiscordHandle = `SELECT rank, discordHandle, qps, matchWins, matchLosses, allTimeRank FROM
    (${selectStandingsBySeason}) standings INNER JOIN player ON standings.playerId = player.id ORDER BY rank ASC`;
exports.selectStandingsWithDiscordHandle = selectStandingsWithDiscordHandle;
...
const selectWishlistCardByName = `SELECT * FROM mtgoCard WHERE name = $cardName and numWishlist > 0`;
exports.selectWishlistCardByName = selectWishlistCardByName;
const selectOwnedCards = `SELECT * FROM mtgoCard WHERE numOwned > 0`;
exports.selectOwnedCards = selectOwnedCards;
const selectWishlistCards = `SELECT * FROM mtgoCard WHERE numWishlist > 0`;
exports.selectWishlistCards = selectWishlistCards;
