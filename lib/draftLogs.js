...
async function processAllEventFiles() {
  const allEventIds = await (0, _db.executeSelectSome)('SELECT id FROM event;', {}).then(rows => rows.map(row => row.id));
  const allFolders = (0, _fs.readdirSync)(EVENT_FOLDER).filter(item => allEventIds.includes(item));
  ...
  for (let eventId of allFolders) {
    const eventPath = _path.default.join(EVENT_FOLDER, eventId);

    const txtFilePaths = (0, _fs.readdirSync)(eventPath).filter(item => /\.txt$/.test(item)).map(item => _path.default.join(eventPath, item));
    const logFilePaths = txtFilePaths.filter(fileIsDraftLog);
    const deckFilePaths = txtFilePaths.filter(fileIsDecklist);
    const seatings = await (0, _db.executeSelectSome)('SELECT playerId FROM entry WHERE eventId = $eventId ORDER BY seatNum ASC', {
      $eventId: eventId
...
async function loadLogAndWrite(filename, eventId, seatings) {
  ...
  const processedLog = processLog((0, _fs.readFileSync)(filename, 'utf-8'));
...
async function loadDeckAndWrite(filename, eventId) {
  ...
  const processedDeck = processDeck((0, _fs.readFileSync)(filename, 'utf-8'));
  let playerId;
  ...
  for (let cardRow of processedDeck.cardRows) {
    ...
    if (matchingRow) {
      await (0, _db2.executeRun)(`UPDATE pick SET isMain = $isMain WHERE playerId = $playerId AND eventId = $eventId AND pickId = $pickId AND isMain IS NULL;`, {
      ...
      await (0, _db2.executeRun)(`INSERT INTO pick(playerId, eventId, pickId, isMain, cardName) VALUES ($playerId, $eventId, $pickId, $isMain, $cardName);`, {
        $playerId: playerId,
        $eventId: eventId,
        $pickId: pickId,
        $isMain: cardRow.isMain,
