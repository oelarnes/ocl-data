...
async function scryfallAsOf() {
  ...
  return mongo.collection('import_metadata').findOne({}).then(item => item === null || item === void 0 ? void 0 : item.as_of).catch(err => {
    console.log(err);
    return;
  });
}
...
async function dataSync() {
  ...
  const dbConfig = (0, _db.getFreshDbConfig)(); // first make sure there are event folders for every event in the config
  ...
  const knownEventIds = await (0, _db.executeSelectSome)(`SELECT id FROM event`, {}, 'id');
...
async function processAllEventFiles() {
  ...
  const allEventIds = [...Set(allEventIdsInDb.concat(Object.keys((0, _db.getFreshDbConfig)().eventSheets)))];
...
async function processOneEvent(eventId) {
  ...
  if (newLogFiles.length > 0) {
    ...
    const seatings = await (0, _db.executeSelectSome)('SELECT playerId FROM entry WHERE eventId = $eventId ORDER BY seatNum ASC AND playerId IS NOT NULL', {
      $eventId: eventId
    }, 'playerId');
...
async function loadLogAndWrite(filename, eventId, seatings) {
  ...
  if (seatings.length === 8) {
    ...
    if (fullName !== undefined) {
      await (0, _db.executeInsertData)(`INSERT INTO entry(eventId, playerId, seatNum) VALUES ($eventId, $playerId, $seatNum)`, {
        $eventId: eventId,
        $playerId: playerId,
        $seatNum: seatNum
      });
